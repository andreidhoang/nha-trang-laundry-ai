...
BASE = "https://staging.internal:8443"

# What the console can actually reach.
PATHS = set()
for js in (ROOT / "apps/web/src").rglob("*.js"):
    for m in re.findall(r"""["'`](/internal/v1[^"'`\s]*)["'`]?""", js.read_text(encoding="utf-8")):
...
client = DemoClient(BASE, ROOT / ".demo/ca.crt")
...
move("thương mại → đã xác nhận", f"/internal/v1/orders/{oid}/transition", {"target": "CONFIRMED"})
...
for target, vi in (
    ...
    move(f"sản xuất → {vi}", f"/internal/v1/orders/{oid}/production-transition", {"target": target})
move(
    "tất toán (settlement)",
