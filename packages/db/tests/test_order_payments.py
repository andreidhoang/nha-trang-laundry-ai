...
from nha_trang_laundry_db.reports import ReportKey, ReportRepository, shop_today
...
from nha_trang_laundry_domain.shop_capture import CORE_MARGIN_CATEGORIES, MarginStatus
...
# --- the month's margin reads the same ledger as the takings (round 7 merge) ---------------------


def test_a_months_margin_takes_its_money_from_the_payment_ledger_each_payment_once(
    connection: psycopg.Connection[Any],
) -> None:
    """`SHOP-CAPTURE-001`'s month money and `PAYMENT-001`'s ledger, merged: the month's takings are
    the payments recorded in it less the refunds, the rule `collected-today-v3` and
    `MONEY_COLLECTED` apply. A deposit taken last month is last month's money; the balance paid this
    month is this month's; neither is counted again through the settlement the balance wrote (which
    carries the whole total, on the balance's day)."""

    zone = ZoneInfo("Asia/Ho_Chi_Minh")
    today = shop_today(datetime.now(UTC))
    this_month = today.replace(day=1)
    last_month_day = this_month - timedelta(days=15)
    last_month = last_month_day.replace(day=1)
    deposit_at = datetime.combine(last_month_day, time(12), tzinfo=zone)

    store_id, staff, order_id = _received(connection)
    _pay(connection, order_id, staff, 30_000, method=TRANSFER, seen=True, at=deposit_at)
    settled = _pay(connection, order_id, staff, TOTAL_VND - 30_000)
    assert (settled.paid_vnd, settled.remaining_vnd) == (TOTAL_VND, 0)
    assert settled.settlement_id is not None
    # A second order: a deposit this month, then a cancellation that refunds it this month.
    cancelled = _order(connection, store_id, staff)
    _step(connection, cancelled, staff, 1, OrderStep.RECEIVE, slot_approved=True)
    _pay(connection, cancelled, staff, 20_000)
    ...
    for category in CORE_MARGIN_CATEGORIES:
    ...
    months = {month.from_date: month for month in report.months}
    assert set(months) == {last_month, this_month}

    def ledger(first: date, last: date) -> tuple[int, int]:
        lower = datetime.combine(first, time(0), tzinfo=zone)
        upper = datetime.combine(last + timedelta(days=1), time(0), tzinfo=zone)
        [(paid,)] = _rows(
            connection,
            "SELECT coalesce(sum(amount_vnd), 0) FROM order_payments "
            "WHERE store_id = %s AND recorded_at >= %s AND recorded_at < %s",
            store_id,
            lower,
            upper,
        )
        [(refunded,)] = _rows(
            connection,
            "SELECT coalesce(sum(refunded_amount_vnd), 0) FROM order_refunds "
            "WHERE store_id = %s AND direction = 'TO_CUSTOMER' "
            "AND refunded_at >= %s AND refunded_at < %s",
            store_id,
            lower,
            upper,
        )
    ...
    before = months[last_month]
    assert (before.collected_vnd, before.refunded_vnd) == (30_000, 0)
    assert (before.collected_vnd, before.refunded_vnd) == ledger(before.from_date, before.to_date)
    assert before.margin.status is MarginStatus.INCOMPLETE and before.margin.amount_vnd is None

    now = months[this_month]
    assert (now.collected_vnd, now.refunded_vnd) == (TOTAL_VND - 30_000 + 20_000, 20_000)
    assert (now.collected_vnd, now.refunded_vnd) == ledger(now.from_date, now.to_date)
    spending = 1_000 * len(CORE_MARGIN_CATEGORIES)
    assert now.spending_vnd == spending
    assert now.margin.status is MarginStatus.COMPLETE
    assert (now.margin.amount_vnd, now.margin.direction) == (
        TOTAL_VND - 30_000 + 20_000 - 20_000 - spending,
        "IN",
    )
    # And the window's MONEY_COLLECTED, the same ledger over the same days, agrees to the đồng.
    figures = {figure.key: figure for figure in report.summary.figures}
    assert figures[ReportKey.MONEY_COLLECTED].numerator == (
        before.collected_vnd + now.collected_vnd
    )
    assert figures[ReportKey.MONEY_REFUNDED].numerator == before.refunded_vnd + now.refunded_vnd
