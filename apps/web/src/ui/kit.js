...
export function moneyInput(spec) {
  const echo = h("p", { class: "money-entry__echo", "aria-live": "polite" });
  const input = /** @type {HTMLInputElement} */ (
    h("input", {
      id: spec.id,
      type: "text",
      class: "money-entry__field",
      inputmode: "numeric",
      autocomplete: "off",
      enterkeyhint: "done",
  ...
  const node = h(
    ...
    h(
      "div",
      { class: "money-entry__box" },
      input,
      h("span", { class: "money-entry__suffix", "aria-hidden": "true" }, "₫"),
    ),
    echo,
  );
  return { node, input };
