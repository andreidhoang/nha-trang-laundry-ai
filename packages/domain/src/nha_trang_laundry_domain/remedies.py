...
class RemedyCommitments:
    ...
    #: migration `0053`: nothing says which shirt it was about, so it is assumed to have been about
...
def committed_against_garment(by_garment: Mapping[int | None, int], garment_index: int) -> int:
    ...
    `None` for a proposal that named none (written before migration `0053`). The line-level sum is
