...
| 6a | Hand the customer a receipt (`RECEIPT-PRINT-001`) | `#/orders/{id}/receipt` | **In phiếu cho khách** on the order page Nhận đồ opens (later: **Khác → In phiếu**), then **In phiếu** or, where the phone can share, **Chia sẻ** | reads only: `GET /internal/v1/orders/{id}`, `GET …/quotes/{id}?revision=` | A slip for an 80 mm / 58 mm roll or A5: the store's registered name, the customer's name when the order has a record, the ticket and its day, each line at its list price, the promotion / credit / delivery fee as their own rows, the server's total, when the order was taken, an 8-character reference. **No ready time** (founder ruling R4): it says "Tiệm sẽ báo khi đồ sẵn sàng" when the shop can reach the customer (a phone on record, or the chat the order came from), and "Giữ phiếu này để nhận đồ" to a walk-in who left only a ticket |
