...
def test_secrets_identities_and_migration_job_are_least_privilege(
    ...
    # 11 since HASH-KEYING-001 added `staging_hash_key`. The count is pinned rather than merely
    # bounded because a secret appearing here is a new thing a deployment must be given, and one
    # appearing without anyone noticing is how a service starts, finds no key and refuses -- which
    # reads as a broken deploy rather than as a missing secret. The key is generated by
    # `generate_demo_material.py` and `bootstrap_shop_local.py`, never typed.
    assert len(secrets) == 11
    assert "staging_hash_key" in secrets
