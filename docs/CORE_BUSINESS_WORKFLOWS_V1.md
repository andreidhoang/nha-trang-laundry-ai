...
- **`#/reports` Báo cáo** (`REPORT-DASHBOARD-001`) is the owner's numbers for a window of shop-local
  ...
  cancelled; on time against each order's **first** promise (`PROMISE-001`; `COMPLETE` when every
  order had one, `RULE_ASSUMED` with a count when some were taken before the turnaround policy and
...
| `docs/STAFF_CONSOLE_ENGINEERING_SPEC_V1.md` §2 | The SLA risk read model is unrouted | Routed since `OPS-BOARD-001`: `GET /internal/v1/stores/{store_id}/sla-board` behind `#/sla-board`. The per-order policy it also names was decided by `DEC-037` and built as `PROMISE-001`: each order's promised-ready time |
