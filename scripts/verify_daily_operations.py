...
with sync_playwright() as pw:
    ...
    def open_create_form():
        """The quote-to-order form on #/orders. Without a hand-off in the address it sits under
        "Nhập mã thủ công" (CONSOLE-REDESIGN-002), so it is opened first, as a person would."""
        page.goto(f"{CONSOLE}#/orders", wait_until="networkidle")
        page.wait_for_timeout(1400)
        manual = page.locator("details.manual-entry")
        if manual.count() and not manual.first.evaluate("(node) => node.open"):
            manual.first.locator(":scope > summary").click()
    ...
    ok(
        ...
        len([h for h in hints if "Vai trò được phép" in h]) >= 1,
        f"{len([h for h in hints if 'Vai trò được phép' in h])} explain the refusal",
    )
