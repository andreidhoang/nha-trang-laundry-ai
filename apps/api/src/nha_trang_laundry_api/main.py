...
class OrderStepRequest(StrictRequest):
    ...
    `TAKE_PAYMENT`, `COLLECT`, `DELIVERY_PICKUP` and `DELIVERY_RETURN` appear in `next_steps` but
    are recorded on their own routes (payments, collection, delivery legs), which carry the facts
    only a person can attest -- the amount taken and how, the courier's outcome.
    """
