...
| Old finding | Status |
|---|---|
| **F1** — the agent runtime is an orphan, 1,216 lines no code path reaches | **Fixed.** `AGENT-PIPELINE-001` complete. A job now traverses queue → claim → runtime → persisted evidence with no manual step; the evidence records this as "the first time a job has traversed queue to persisted runtime evidence in this repository." Claim exclusivity is proven under a real two-worker race. Flags remain closed by default and the assembled transport declares `provider_backed: false`. |
