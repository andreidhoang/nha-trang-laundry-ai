...
export function render_(context) {
  ...
  function openReceive(entry) {
    ...
    const made = openFresh({
      ...
      body: h(
        ...
        h(
          "label",
          { class: "check-row", for: "receive-slot" },
          tick,
      ...
      render(
        ...
        items.length
          ? list(
              items.map((item) =>
                listRow({
                  href: `#/remedies?incident=${encodeURIComponent(String(item.incident_id))}`,
                  leading: "incident",
  ...
  return h(
      ...
      info: infoButton(
        ...
        h(
          ...
          "Bồi hoàn gắn với khiếu nại, không gắn thẳng với đơn: mở một khiếu nại trong danh sách " +
            "này rồi bấm “Đề xuất bồi hoàn” trên chính khiếu nại đó.",
        ),
      ),
    ...
    section({
      ...
      card: false,
      action: linkButton({ href: "#/remedies", label: "Dùng mã", variant: "quiet" }),
      info: infoButton(
