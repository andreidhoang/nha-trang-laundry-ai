...
1. A claim on a line priced per piece names its garment (`garment_index`, 1..quantity), and the
   100.000 d staff limit and the 5x ceiling are cumulative per (line, garment). Migration `0053`
...
def _copied_row(
    ...
    *,
    ...
    With `garment=None` it is exactly what a database deployed before migration 0053 holds: same
...
# --- 2. rows written before migration 0053 ------------------------------------------------------
